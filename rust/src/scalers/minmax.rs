...
impl MinMaxScaler {
    ...
    fn get_params(&self, py: Python<'_>, deep: bool) -> std::collections::HashMap<String, PyObject> {
        ...
        params.insert("feature_range".to_string(), self.feature_range.into_pyobject(py).unwrap().unbind().into());
        params.insert("clip".to_string(), pyo3::types::PyBool::new(py, self.clip).to_owned().unbind().into());
